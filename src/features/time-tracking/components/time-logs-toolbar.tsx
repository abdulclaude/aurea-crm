...
import { IconClockAlert as DurationIcon } from "central-icons/IconClockAlert";
...
import { IconPound as AmountIcon } from "central-icons/IconPound";
import { IconSettingsSliderThree as FilterIcon } from "central-icons/IconSettingsSliderThree";
import { IconSuitcaseWork as JobIcon } from "central-icons/IconSuitcaseWork";
...
export interface TimeLogsToolbarProps {
  search: string;
...
export function TimeLogsToolbar({
  search,
  onSearchChange,
  sortValue,
  onSortChange,
  ...
  // Calculate min/max for duration, amount, and dates + histograms
  const {
    minDuration,
    maxDuration,
    minAmount,
    ...
    const maxDur = durations.length > 0 ? Math.max(...durations) : 480; // 8 hours
    const minAmt = amounts.length > 0 ? Math.min(...amounts) : 0;
    const maxAmt = amounts.length > 0 ? Math.max(...amounts) : 1000;

    // Build histograms (40 bins)
    const numBins = 40;
    const durationHist = new Array(numBins).fill(0);
    const amountHist = new Array(numBins).fill(0);

    // Duration histogram
    if (durations.length > 0 && maxDur > minDur) {
      durations.forEach((dur) => {
        const binIndex = Math.min(
          numBins - 1,
          Math.floor(((dur - minDur) / (maxDur - minDur)) * numBins),
        );
        durationHist[binIndex]++;
      });
    }

    // Amount histogram
    if (amounts.length > 0 && maxAmt > minAmt) {
      amounts.forEach((amt) => {
        const binIndex = Math.min(
          numBins - 1,
          Math.floor(((amt - minAmt) / (maxAmt - minAmt)) * numBins),
        );
        amountHist[binIndex]++;
      });
    }
