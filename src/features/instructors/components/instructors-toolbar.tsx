...
export interface InstructorsToolbarProps {
  search: string;
...
export function InstructorsToolbar({
  ...
  // Fetch all instructors for filter options and preview
  const { data: allInstructorsData } = useSuspenseQuery(
    trpc.instructors.list.queryOptions({}),
  );

  const allInstructors = React.useMemo(
    () => allInstructorsData?.items || [],
    [allInstructorsData],
  );
  ...
  const { minRate, maxRate, minDate, maxDate, rateHistogram } =
    React.useMemo(() => {
      const rates = allInstructors
        .map((instructor) =>
          instructor.hourlyRate ? Number(instructor.hourlyRate) : null,
        )
        .filter((r): r is number => r !== null);
      const dates = allInstructors
        .map((instructor) => new Date(instructor.createdAt))
  ...
  const previewCount = React.useMemo(() => {
    const filtered = allInstructors.filter((instructor) => {
      ...
      if (stagedRoles.length > 0) {
        if (!instructor.role || !stagedRoles.includes(instructor.role)) {
          return false;
        }
      }
      ...
      if (stagedStatus !== null) {
        if (instructor.isActive !== stagedStatus) {
          return false;
        }
      }
      ...
      if (
        ...
        const rate = instructor.hourlyRate ? Number(instructor.hourlyRate) : 0;
  ...
  }, [allInstructors, stagedRoles, stagedStatus, stagedRateMin, stagedRateMax]);
    ...
    <div className="flex justify-between w-full items-center py-4">
      <div className="flex items-center gap-2 w-full">
        ...
        <div className="flex w-128 items-center bg-background transition duration-250 relative hover:bg-primary-foreground/50 hover:text-black rounded-lg h-8.5">
          ...
          <Input
            placeholder="Search instructors by name, email, role..."
            value={searchInput}
        ...
        {scope === "all-clients" && onLocationIdChange && (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
            ...
            >
              ...
              {clients.map((client: any) => (
                <DropdownMenuCheckboxItem
                  ...
                  onSelect={() => onLocationIdChange(client.locationId)}
                  className="px-10 py-2.5 text-xs bg-background text-primary/80 hover:bg-primary-foreground/50 hover:text-black cursor-pointer"
                >
                  {client.name}
                </DropdownMenuCheckboxItem>
