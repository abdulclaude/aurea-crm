...
import type { IntroOfferWidgetConfig } from "@/features/studio/widgets/contracts";

export function IntroOfferDisplayFields({
  idPrefix,
  config,
  options,
  onChange,
  ...
  onChange: (patch: Partial<IntroOfferWidgetConfig>) => void;
}) {
  ...
  return (
    <>
      <div className="space-y-2">
        ...
        <Select
          value={config.layout}
          onValueChange={(layout: IntroOfferWidgetConfig["layout"]) =>
            onChange({ layout })
          }
        >
      ...
      <div className="space-y-2">
        <Label htmlFor={`${idPrefix}-featured`}>Featured offer</Label>
        <Select
      ...
      <div className="space-y-2">
        <Label htmlFor={`${idPrefix}-button`}>Button label</Label>
        <Input
          id={`${idPrefix}-button`}
