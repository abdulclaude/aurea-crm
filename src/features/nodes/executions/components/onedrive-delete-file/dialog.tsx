...
export type OnedriveDeleteFileFormValues = z.infer<typeof formSchema>;

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (values: OnedriveDeleteFileFormValues) => void;
  defaultValues?: Partial<OnedriveDeleteFileFormValues>;
  variables: VariableItem[];
}

export const OnedriveDeleteFileDialog: React.FC<Props> = ({
  open,
  onOpenChange,
  onSubmit,
  defaultValues = {},
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <ResizableSheetContent className="overflow-y-auto sm:max-w-xl bg-background border-border">
        <SheetHeader className="px-6 pt-8 pb-1 gap-1">
          <SheetTitle>OneDrive: Delete File Configuration</SheetTitle>
          <SheetDescription>
