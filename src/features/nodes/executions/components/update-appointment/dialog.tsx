...
export type UpdateAppointmentFormValues = z.infer<typeof formSchema>;

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (values: UpdateAppointmentFormValues) => void;
  defaultValues?: Partial<UpdateAppointmentFormValues>;
  variables: VariableItem[];
}

export const UpdateAppointmentDialog: React.FC<Props> = ({
  open,
  onOpenChange,
  onSubmit,
  defaultValues = {},
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      variableName: defaultValues.variableName || "updatedAppointment",
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <ResizableSheetContent className="overflow-y-auto sm:max-w-xl bg-background border-border">
        <SheetHeader className="px-6 pt-8 pb-1 gap-1">
          <SheetTitle>Update Appointment Configuration</SheetTitle>
          <SheetDescription>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="updatedAppointment" {...field} />
                  </FormControl>
                  <FormDescription>
