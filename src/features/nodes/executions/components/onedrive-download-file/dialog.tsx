...
export type OnedriveDownloadFileFormValues = z.infer<typeof formSchema>;

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (values: OnedriveDownloadFileFormValues) => void;
  defaultValues?: Partial<OnedriveDownloadFileFormValues>;
  variables: VariableItem[];
}

export const OnedriveDownloadFileDialog: React.FC<Props> = ({
  open,
  onOpenChange,
  onSubmit,
  defaultValues = {},
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <ResizableSheetContent className="overflow-y-auto sm:max-w-xl bg-background border-border">
        <SheetHeader className="px-6 pt-8 pb-1 gap-1">
          <SheetTitle>OneDrive: Download File Configuration</SheetTitle>
          <SheetDescription>
