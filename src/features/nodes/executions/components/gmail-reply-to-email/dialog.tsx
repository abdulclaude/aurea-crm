...
const formSchema = requiredWorkflowProviderBindingSchema.extend({
  ...
  replyBody: z.string().min(1, { message: "Reply body is required." }),
});

export type GmailReplyToEmailFormValues = z.infer<typeof formSchema>;

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (values: GmailReplyToEmailFormValues) => void;
  defaultValues?: Partial<GmailReplyToEmailFormValues>;
  variables: VariableItem[];
}

export const GmailReplyToEmailDialog: React.FC<Props> = ({
  open,
  onOpenChange,
  onSubmit,
  defaultValues = {},
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      variableName: defaultValues.variableName || "gmailReply",
      messageId: defaultValues.messageId || "",
      replyBody: defaultValues.replyBody || "",
    },
  });
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <ResizableSheetContent className="overflow-y-auto sm:max-w-xl bg-background border-border">
        <SheetHeader className="px-6 pt-8 pb-1 gap-1">
          <SheetTitle>Gmail reply to email</SheetTitle>
          <SheetDescription>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="gmailReply" {...field} />
                  </FormControl>
                  <FormDescription className="text-[11px] mt-1">
                    Access the result:{" "}
                    <span className="text-primary font-medium">
                      @{field.value || "gmailReply"}
                    </span>
                  </FormDescription>
                  <FormMessage />
                </FormItem>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <VariableInput
                      ...
                      placeholder="@gmailMessage.id"
                      variables={variables}
                    />
                  </FormControl>
                  <FormDescription className="text-[11px] mt-1">
                    The ID of the email message to reply to
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription className="text-[11px] mt-1">
                    The body of your reply (supports HTML)
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
