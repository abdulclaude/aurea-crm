...
export type ClientFieldChangedTriggerFormValues = z.infer<typeof formSchema>;

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (values: ClientFieldChangedTriggerFormValues) => void;
  defaultValues?: Partial<ClientFieldChangedTriggerFormValues>;
  variables: VariableItem[];
}

export const ClientFieldChangedTriggerDialog: React.FC<Props> = ({
  open,
  onOpenChange,
  onSubmit,
  defaultValues = {},
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      variableName: defaultValues.variableName || "clientChange",
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <ResizableSheetContent className="overflow-y-auto sm:max-w-xl bg-background border-border">
        <SheetHeader className="px-6 pt-8 pb-1 gap-1">
          <SheetTitle>Client Field Changed Trigger Configuration</SheetTitle>
          <SheetDescription>
            This workflow will trigger when a specific client field changes.
          </SheetDescription>
        </SheetHeader>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="clientChange" {...field} />
                  </FormControl>
                  <FormDescription className="text-xs mt-2 leading-5">
                    Access the change data in other nodes: <br />
                    <span className="text-primary font-medium tracking-wide">
                      {`{{${field.value || "clientChange"}.oldValue}}`}
                    </span>
                    {", "}
                    <span className="text-primary font-medium tracking-wide">
                      {`{{${field.value || "clientChange"}.newValue}}`}
                    </span>
                  </FormDescription>
                  <FormMessage />
                </FormItem>
