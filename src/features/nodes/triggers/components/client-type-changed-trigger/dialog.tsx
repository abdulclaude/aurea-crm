...
export type ClientTypeChangedTriggerFormValues = z.infer<typeof formSchema>;

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (values: ClientTypeChangedTriggerFormValues) => void;
  defaultValues?: Partial<ClientTypeChangedTriggerFormValues>;
  variables: VariableItem[];
}

export const ClientTypeChangedTriggerDialog: React.FC<Props> = ({
  open,
  onOpenChange,
  onSubmit,
  defaultValues = {},
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      variableName: defaultValues.variableName || "clientTypeChange",
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <ResizableSheetContent className="overflow-y-auto sm:max-w-xl bg-background border-border">
        <SheetHeader className="px-6 pt-8 pb-1 gap-1">
          <SheetTitle>Client Type Changed Trigger Configuration</SheetTitle>
          <SheetDescription>
            This workflow will trigger when a client type changes.
          </SheetDescription>
        </SheetHeader>
          ...
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="clientTypeChange" {...field} />
                  </FormControl>
                  <FormDescription className="text-xs mt-2 leading-5">
                    Access the type change data in other nodes: <br />
                    <span className="text-primary font-medium tracking-wide">
                      {`{{${field.value || "clientTypeChange"}.oldType}}`}
                    </span>
                    {", "}
                    <span className="text-primary font-medium tracking-wide">
                      {`{{${field.value || "clientTypeChange"}.newType}}`}
                    </span>
                  </FormDescription>
                  <FormMessage />
                </FormItem>
            ...
            <div className="bg-blue-500/10 border border-blue-500/20 rounded p-4">
              <p className="text-sm text-blue-200">
                Useful for automations when a client moves from Lead to Customer, or
