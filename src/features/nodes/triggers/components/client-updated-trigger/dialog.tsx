...
export type ClientUpdatedTriggerFormValues = z.infer<typeof formSchema>;

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (values: ClientUpdatedTriggerFormValues) => void;
  defaultValues?: Partial<ClientUpdatedTriggerFormValues>;
  variables: VariableItem[];
}

export const ClientUpdatedTriggerDialog: React.FC<Props> = ({
  open,
  onOpenChange,
  onSubmit,
  defaultValues = {},
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      variableName: defaultValues.variableName || "updatedClient",
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <ResizableSheetContent className="overflow-y-auto sm:max-w-xl bg-background border-border">
        <SheetHeader className="px-6 pt-8 pb-1 gap-1">
          ...
          <SheetDescription>
            This workflow will trigger whenever a client is updated in your CRM.
          </SheetDescription>
        </SheetHeader>
          ...
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="updatedClient" {...field} />
                  </FormControl>
            ...
            <div className="bg-blue-500/10 border border-blue-500/20 rounded p-4">
              <p className="text-sm text-blue-200">
                This trigger will fire automatically whenever a client is updated. The
