...
export type ClientDeletedTriggerFormValues = z.infer<typeof formSchema>;

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (values: ClientDeletedTriggerFormValues) => void;
  defaultValues?: Partial<ClientDeletedTriggerFormValues>;
  variables: VariableItem[];
}

export const ClientDeletedTriggerDialog: React.FC<Props> = ({
  open,
  onOpenChange,
  onSubmit,
  defaultValues = {},
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      variableName: defaultValues.variableName || "deletedClient",
    },
  });
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <ResizableSheetContent className="overflow-y-auto sm:max-w-xl bg-background border-border">
        <SheetHeader className="px-6 pt-8 pb-1 gap-1">
          ...
          <SheetDescription>
            This workflow will trigger whenever a client is deleted from your CRM.
          </SheetDescription>
        </SheetHeader>
          ...
          >
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="deletedClient" {...field} />
                  </FormControl>
            ...
            <div className="bg-orange-500/10 border border-orange-500/20 rounded p-4">
              <p className="text-sm text-orange-200">
                This trigger fires when a client is permanently deleted. Useful for
